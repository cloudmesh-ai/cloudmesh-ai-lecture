...
!!! info "Learning Objectives"
    ...
    1. **Differentiate** between the configuration models of the Continue and Cline extensions.
    2. **Configure** the Continue extension using both the GUI and the `config.yaml` file.
    3. **Set up** local LLM providers (e.g., Ollama) and cloud providers (e.g., OpenRouter) within VS Code.
    4. **Provision** Cline as an autonomous agent capable of file system and terminal operations.
    5. **Optimize** model performance by adjusting temperature, context windows, and prompt settings.

...
### 4.7 Safe Persistence – Back‑ups & Context Managers  
...
Answer the following questions on your own (write short code snippets or prose).  
Check the **expected answer** after you try; you can run the snippets in the same notebook.

1. **Create** a new YAMLDB called `students.yaml`. Insert three student records with fields `id`, `name`, `gpa`.  
2. **Query** for all students with a GPA **≥ 3.5** and sort them by `name`.  
3. **Update** the GPA of the student whose `id` is `2` by adding `0.2`.  
4. **Delete** any student whose GPA falls below `2.0`.  

*Write the code, execute it, and then copy the final contents of `students.yaml` into the answer box below.*  

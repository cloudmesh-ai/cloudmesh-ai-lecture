...
> **In short:** the proxy route uses the public API gateway and needs your token; the direct route hits the native vLLM/SGLang server without authentication.
...
> **⚠️ Warning** – Do **not** use Option B unless you are prepared for GPU charges.  
> **💡 Instant LLM chat** – For most users the zero‑cost central service at <https://llm.jetstream-cloud.org> is the fastest way to start querying Llama 4 Scout, DeepSeek‑R1, or GPT‑OSS‑120B.
...
2. **API token (JWT)** generated from the Open WebUI – see the “🗝️ Obtaining an Open WebUI API Token” section below.  
...
## 🗝️ Obtaining an Open WebUI API Token (Option A)
...
### 2️⃣ Navigate to the token‑creation page  
...
### 4️⃣ Safely storing the token locally  
...
### 1️⃣ Access via the **Open WebUI proxy** (public, token‑protected)
...
```bash
http POST https://llm.jetstream-cloud.org/api/v1/chat/completions \
    Authorization:"Bearer <your-token>" \
    model=llama-4-scout \
    messages:='[{"role":"user","content":"Say hello"}]'
```

### 2️⃣ Direct backend endpoints (internal network or SSH tunnel)
...
> **⚠️ CORS / Network note** – These URLs are reachable **only from Jetstream’s internal network** (or via an SSH tunnel). If your client library requires an `apiKey`, any non‑empty string (e.g. `"dummy"` ) will satisfy it.

---

## Option B – Self‑Hosting on a Jetstream 2 GPU Instance

### 1️⃣ Provision the GPU VM
...
### 2️⃣ Install the serving stack (example with `llama.cpp`)
...
### 3️⃣ Run the OpenAI‑compatible server
...
### 4️⃣ Expose the service over HTTPS (recommended)
...
> **Authentication rule** –  
> * **Proxy endpoints**: set `apiKey` to the JWT you generated in the *Prerequisites* step.  
...
1. Put the token into `~/.ssh/jetstream.env` as shown in **Section 🗝️** and source the file or export `JETSTREAM_API_TOKEN`.  
2. In the JSON, set `apiKey` to an empty string:
  ...
  "models": [
    {
      ...
      "model": "llama-4-scout",
      "apiBase": "https://llm.jetstream-cloud.org/api/v1",
      "apiKey": ""          // Continue will fall back to $JETSTREAM_API_TOKEN
    }
  ]
}
```

Continue’s Python SDK (and the VS Code extension) will read `process.env.JETSTREAM_API_TOKEN` (or `OPENAI_API_KEY`) when `apiKey` is empty.
...
| **API Key** | Real JWT (proxy) <br>or `dummy` / blank (direct/self‑hosted) |
...
Cline also reads a workspace‑level `settings.json`. You can reference the environment variable directly:
...
If the `${env:…}` placeholder cannot be resolved, Cline will fall back to an empty string, which will produce a `401` – make sure the environment variable is exported before launching VS Code.
...
| **401 Unauthorized** on central API | Missing, malformed, or expired JWT | Regenerate a fresh token from *Chat Settings → API Tokens* (or follow the steps in **🗝️ Obtaining an Open WebUI API Token**). |
...
A valid response contains a `choices` array. If you get `401`, double‑check the token’s expiry and regenerate it.
...
* **Llama 4 Scout**
    ...
    * **Model ID:** `llama-4-scout` (or check exact string from documentation)
...
5. Fill in the connection details:
    ...
    * **API Key:** Type `empty` or any random placeholder string (the proxy endpoints do not enforce strict auth token checks, but the field usually requires a non-empty string input).
...
And then configure Cline to point to `http://localhost:8000/v1` with an OpenAI-compatible provider. Otherwise, for the public multi-tenant service, the direct HTTPS URL works smoothly out of the box.
...
??? question "When would a developer choose the 'minimalist core' installation of FastAPI?"
    A developer might choose the minimalist core if they want total control over their environment, intend to use a different ASGI server (like Hypercorn), or are building a highly optimized Docker container where reducing the number of installed packages is critical for size and security.

...
??? question "Contrast HTTP Basic Auth with API Key authentication in terms of use cases."
    **HTTP Basic Auth** is simple and built-in to browsers, making it suitable for lightweight internal tools or simple password-protected pages. **API Key authentication** is better for server-to-server communication or external API clients, as keys are more easily rotated, revoked, and managed.

??? question "How does a TOTP-based 2FA system add security beyond a standard password?"
    TOTP (Time-based One-Time Password) adds a second layer of verification by requiring a code that changes every 30 seconds. Even if an attacker steals the user's password, they cannot access the account without the physical device (e.g., a phone with Google Authenticator) that generates the current TOTP code.

??? question "Why is it a security best practice to put a reverse proxy (like Caddy or Nginx) in front of a FastAPI service?"
    A reverse proxy handles several critical production tasks that the application server (Uvicorn) is not designed for, including SSL/TLS termination (HTTPS), request filtering, load balancing, and protecting the application server from direct exposure to the public internet.

??? question "In a production Docker Compose setup, why should the API container have no open ports exposed to the host?"
    By not exposing ports, the API container is only accessible through the internal Docker network. This forces all external traffic to pass through the reverse proxy (like Caddy), ensuring that security policies, SSL termination, and logging are applied to every single request.

??? question "Explain the benefit of using `secrets.compare_digest` when verifying passwords."
    `secrets.compare_digest` performs a constant-time string comparison. This prevents "timing attacks," where an attacker could potentially deduce the correct password by measuring the exact time it takes for the server to reject an incorrect password.
